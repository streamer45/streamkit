...
| `wasm_call_timeout_secs` | integer | null (uint64) | `300` | WASM plugin per-call execution timeout in seconds (default: 300, minimum: 1). Bounds a single guest invocation; a plugin that exceeds it is interrupted via wasmtime epoch interruption and the node fails. Set to `null` to disable the per-call deadline entirely. Values below 1 are clamped to 1 to prevent instant timeouts. |
