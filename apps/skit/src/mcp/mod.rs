...
impl StreamKitMcp {
    ...
    #[tool(
        description = "Apply a batch of graph mutations to a running session as a single validated batch. All operations are validated before any are applied; if validation fails, none are applied. Added nodes use confirmed-add semantics: they appear in the pipeline only after the engine constructs them, so a node whose creation fails after validation leaves no orphan node entry (failures surface as a Failed node state), and a connection recorded for a node whose creation then fails is pruned once that failure arrives. Engine-side errors do not roll back other already-applied operations (e.g. connects/removes). Operations: addnode, removenode, connect, disconnect."
    )]
