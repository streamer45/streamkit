...
impl StreamKitMcp {
    ...
    async fn validate_batch(
        ...
        let errors = crate::server::validate_batch_operations(
            ...
            &args.operations,
            &perms,
            self.app_state.file_security_policy(),
        )
        .await;
    ...
    #[tool(
        description = "Apply a batch of graph mutations to a running session as a single validated batch. All operations are validated before any are applied; if validation fails, none are applied. Added nodes use confirmed-add semantics: they appear in the pipeline only after the engine constructs them, so a node whose creation fails after validation leaves no orphan node entry (failures surface as a Failed node state), and a connection recorded for a node whose creation then fails is pruned once that failure arrives. A node in a terminal state — failed, or stopped after completing — is retained by the engine until explicitly removed; re-adding that id without removing it first is rejected (remove it first) rather than silently dropped. Engine-side errors do not roll back other already-applied operations (e.g. connects/removes). Operations: addnode, removenode, connect, disconnect."
    )]
