...
describe('useEdgeAlertSubscription', () => {
  it('returns topoEffectRanRef that gates edge patching', async () => {
  ...
  it('patches edges with alert data when a node enters slow_input_timeout', async () => {
    const { options, getEdges } = makeOptions();
  ...
  it('clears edge alert when node recovers from degraded state', async () => {
    const { options, getEdges } = makeOptions();
  ...
  it('does not patch edges for a non-matching target handle', async () => {
    const edges: Edge[] = [
      {
        id: 'source-mixer',
        source: 'source',
  ...
  it('uses React.startTransition for edge updates', async () => {
