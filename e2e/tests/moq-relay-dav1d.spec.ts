...
/**
 * E2E tests for C dav1d AV1 decode → compositor → SVT-AV1 encode relay pipeline.
 *
 * These tests validate the full AV1 video processing path using the C dav1d
 * decoder (instead of rav1d):
 *   colorbars → AV1 encode (rav1e) → relay → AV1 decode (dav1d) →
 *   compositor → pixel_convert → SVT-AV1 encode → relay
 *
 * The pipeline is self-contained (no browser or webcam needed) and exercises
 * the dav1d decoder and SVT-AV1 encoder nodes through a MoQ relay.
 *
 * Prerequisites:
 * - skit built with --features dav1d_static,svt_av1
...
test.describe('MoQ Relay — dav1d Compositor Pipeline (SVT-AV1)', () => {
  ...
  test('creates dav1d compositor pipeline, verifies dav1d decode and SVT-AV1 encode nodes reach running state', async ({
    baseURL,
  }) => {
    ...
    const sessionName = `relay-dav1d-compositor-test-${Date.now()}`;
    ...
    // dav1d decoder (C dav1d) should exist
    expect(
      nodeKinds.some((n) => n.kind === 'video::dav1d::decoder'),
      'Expected a video::dav1d::decoder node'
    ).toBe(true);
