...
//! Benchmark for the AV1 compositing oneshot pipeline.
//!
//! Runs a graph similar to `samples/pipelines/oneshot/video_compositor_demo.yml`
//! but with an AV1 encoder (rav1e) instead of VP9:
//!
//!   colorbars_bg (NV12) ──┐
//!                        ├─► compositor (fused NV12 output) ──► av1_encoder ──► webm_muxer ──► http_output
//!   colorbars_pip (NV12) ┘
//!
...
//!
//! ```bash
//! cargo bench -p streamkit-engine --bench av1_compositor_pipeline
//! ```
//!
//! Custom frame count / resolution / AV1 params:
//!
//! ```bash
//! cargo bench -p streamkit-engine --bench av1_compositor_pipeline -- --frames 300 --width 640 --height 480 --speed 10 --quantizer 80
//! ```
//!
//! Attach a profiler (e.g. `perf`, `samply`, `cargo flamegraph`):
//!
//! ```bash
//! cargo build --release -p streamkit-engine --bench av1_compositor_pipeline
//! samply record target/release/deps/av1_compositor_pipeline-* -- --frames 300
//! ```
...
/// Default AV1 encoder parameters.
const DEFAULT_AV1_SPEED: u32 = 10;
const DEFAULT_AV1_BITRATE: u32 = 0; // CQ mode
const DEFAULT_AV1_QUANTIZER: u32 = 80;
const DEFAULT_AV1_THREADS: u32 = 0; // auto-detect
...
impl BenchArgs {
    fn parse() -> Self {
        ...
        let mut cfg = Self {
            ...
            iterations: 3,
            speed: DEFAULT_AV1_SPEED,
            bitrate: DEFAULT_AV1_BITRATE,
            quantizer: DEFAULT_AV1_QUANTIZER,
            threads: DEFAULT_AV1_THREADS,
        };
        let mut i = 1;
        while i < args.len() {
            match args[i].as_str() {
...
/// Build the AV1 compositor pipeline definition programmatically.
fn build_pipeline(args: &BenchArgs) -> streamkit_api::Pipeline {
    ...
    // --- AV1 encoder (rav1e) ---
    nodes.insert(
        "av1_encoder".to_string(),
        Node {
            kind: "video::av1::encoder".to_string(),
            params: Some(serde_json::json!({
                "speed": args.speed,
    ...
    let connections = vec![
        ...
        Connection {
            ...
            from_pin: "out".to_string(),
            to_node: "av1_encoder".to_string(),
            to_pin: "in".to_string(),
            mode: streamkit_api::ConnectionMode::Reliable,
        },
        Connection {
            from_node: "av1_encoder".to_string(),
            from_pin: "out".to_string(),
    ...
    Pipeline {
        name: Some("AV1 Compositor Benchmark".to_string()),
        description: Some(format!(
            "Benchmark: {}×{} @ {} fps, {} frames (AV1 speed={})",
...
async fn run_once(engine: &Engine, args: &BenchArgs) -> IterResult {
    let definition = build_pipeline(args);
...
fn main() {
    ...
    eprintln!();
    eprintln!("  AV1 Compositor Pipeline Benchmark");
    eprintln!("  ---------------------------------");
    ...
    eprintln!("  AV1 speed  : {}", args.speed);
    eprintln!("  AV1 bitrate: {} kbps (0=CQ mode)", args.bitrate);
    eprintln!("  AV1 quant. : {}", args.quantizer);
    eprintln!("  AV1 threads: {} (0=auto)", args.threads);
    eprintln!();
    ...
    let json = serde_json::json!({
        "benchmark": "av1_compositor_pipeline",
        "width": args.width,
        "height": args.height,
        "fps": args.fps,
        "frame_count": args.frame_count,
        "iterations": args.iterations,
        "av1_speed": args.speed,
        "av1_bitrate_kbps": args.bitrate,
        "av1_quantizer": args.quantizer,
        "av1_threads": args.threads,
        "mean_secs": mean,
        "min_secs": min,
        "max_secs": max,
        "stddev_secs": stddev,
