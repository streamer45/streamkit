...
use super::super::*;
use crate::dynamic_actor::{DynamicEngine, NodeMetricLabels, NodePinMetadata};
...
/// Mirrors the helper in `pipeline_activation.rs`.
...
async fn stats_fallback_labels_when_cache_missing() {
    ...
    // Should not panic; stats should be stored correctly.
    engine.handle_stats_update(&update);
