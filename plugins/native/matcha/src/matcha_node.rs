...
/// Key: (model_dir, num_threads, execution_provider, noise_scale_bits, length_scale_bits)
...
static TTS_ENGINE_CACHE: std::sync::LazyLock<
    Mutex<HashMap<(String, i32, String, u32, u32), CachedTtsEngine>>,
...
unsafe impl Send for MatchaTtsNode {}
unsafe impl Sync for MatchaTtsNode {}

impl NativeProcessorNode for MatchaTtsNode {
    ...
    fn new(params: Option<serde_json::Value>, logger: Logger) -> Result<Self, String> {
        plugin_info!(logger, "MatchaTtsNode::new() called with params: {:?}", params);

        let config: MatchaTtsConfig = if let Some(p) = params {
            plugin_info!(
                logger,
            ...
            MatchaTtsConfig::default()
        };

        plugin_info!(
            logger,
            "📋 Config parsed successfully: model_dir={}, speaker_id={}, speed={}, noise_scale={}, length_scale={}, num_threads={}, execution_provider={}",
            config.model_dir,
            config.speaker_id,
            config.speed,
            config.noise_scale,
        ...
        // Note: noise_scale and length_scale are set at engine creation time
        // (not at generation time), so they must be in the cache key
        let cache_key = (
            model_dir_str.clone(),
            config.num_threads,
            normalized_provider,
            // Convert to u32 bits for cache key (f32 doesn't implement Eq/Hash)
            config.noise_scale.to_bits(),
            config.length_scale.to_bits(),
        );
    ...
    fn update_params(&mut self, params: Option<serde_json::Value>) -> Result<(), String> {
        if let Some(p) = params {
            let new_config: MatchaTtsConfig =
                serde_json::from_value(p).map_err(|e| format!("Config parse error: {e}"))?;

            // Update runtime parameters (can be changed on-the-fly)
            self.config.speaker_id = new_config.speaker_id;
            self.config.speed = new_config.speed;

            // Note: noise_scale and length_scale are set at engine creation time
            // and cannot be changed at runtime. To change these, you need to
            // create a new node instance with different values.
        }
...
impl MatchaTtsNode {
    fn generate_and_send(&mut self, text: &str, output: &OutputSender) -> Result<(), String> {
        ...
        unsafe {
            ...
            // Use the sample rate from the audio response (typically 22050 for Matcha)
            // Allow: Sample rate from FFI is guaranteed positive (22050 Hz)
            #[allow(clippy::cast_sign_loss)]
...
unsafe fn create_tts_engine(
    ...
    // Matcha model files
    let acoustic_model = model_dir.join("model-steps-3.onnx");
    let vocoder = model_dir.join("vocos-22khz-univ.onnx");
    ...
    for (name, path) in
        [("acoustic_model", &acoustic_model), ("vocoder", &vocoder), ("tokens", &tokens_path)]
    {
        if !path.exists() {
    ...
    if config.execution_provider != "cpu" {
        plugin_warn!(logger, "=== CRITICAL: Calling ONNX Runtime with GPU provider ===");
        plugin_warn!(logger, "If this crashes with 'foreign exception', it means:");
        plugin_warn!(logger, "1. ONNX Runtime threw a C++ exception");
        plugin_warn!(
            logger,
            "2. Likely cause: GPU provider not available in this ONNX Runtime build"
        );
