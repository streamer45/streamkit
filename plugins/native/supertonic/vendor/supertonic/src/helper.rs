// SPDX-FileCopyrightText: © 2025 Supertone, Inc.
//
// SPDX-License-Identifier: MIT
//
// Vendored from https://github.com/supertone-inc/supertonic
// with CLI-specific code removed for library use.
...
pub fn preprocess_text(text: &str, lang: &str) -> Result<String> {
    ...
    if !text.is_empty() {
        let ends_with_punct = Regex::new(
            r#"[.!?;:,'"\u{201C}\u{201D}\u{2018}\u{2019})\]}\u{2026}\u{3002}\u{300D}\u{300F}\u{3011}\u{3009}\u{300B}\u{203A}\u{00BB}]$"#,
        )
        .unwrap();
...
pub fn load_text_to_speech(onnx_dir: &str, use_gpu: bool) -> Result<TextToSpeech> {
    ...
    let dp_path = format!("{onnx_dir}/duration_predictor.onnx");
    let text_enc_path = format!("{onnx_dir}/text_encoder.onnx");
    let vector_est_path = format!("{onnx_dir}/vector_estimator.onnx");
    let vocoder_path = format!("{onnx_dir}/vocoder.onnx");
    ...
    let unicode_indexer_path = format!("{onnx_dir}/unicode_indexer.json");
